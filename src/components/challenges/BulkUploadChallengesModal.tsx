...
export function BulkUploadChallengesModal({ isOpen, onClose }: BulkUploadChallengesModalProps) {
      ...
      try {
        ...
        jsonData.forEach((row: any) => {
            ...
            if (challenge.type === 'MultipleChoice') {
                const qText = row.Question || row.question || "Untitled Question";
                ...
                if (qText && rawOptions && correctAnswer) {
                    challenge.questions?.push({
                        text: qText,
                        options: rawOptions.toString().split(',').map((o: string) => o.trim()),
  ...
  const handleSubmit = () => {
    ...
    console.log("Bulk creating challenges:", parsedChallenges);
    onClose();
  };
